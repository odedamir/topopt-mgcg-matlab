...
%%%% With matrix-free approximate reanalysis %%%%
...
% To reproduce the result tagged "MinV-MFRE5" in Table 2 of the paper, run:
% minW3dmgcg_reMF(48,24,24,3,sqrt(3),1,4,1e-10,100,3330,'minW3dmgcg_reMF')
function minW3dmgcg_reMF(nelx,nely,nelz,penal,rmin,ft,nl,cgtol,cgmax,compconst,filename)
tic;
%% MATERIAL PROPERTIES
E0 = 1;
Emin = 1e-9;
...
while (change > 1e-2 && loop < 50)
    ...
    if (count == 49 || mgcgr > 1e-2)
        ...
        [mgcgi,mgcgr,U] = mgcgMatrixFree(F,U,Kpre,Lfac,Ufac,Pu,nl,1,1e-3,5,...
             xPhys,KE,Emin,E0,penal,edofMat,Null);
        count = count + 1;
    end   
